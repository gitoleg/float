...
let fin t = match t.data with
  | Fin {expn; frac} -> Some (Bignum.z expn, Bignum.z frac)
  | _ -> None

let frac t = match t.data with
  | Fin {frac} -> Some (Bignum.z frac)
  | Nan (_,frac) -> Some (Bignum.z frac)
  | _ -> None

let expn t = match t.data with
  | Fin {expn} -> Some (Bignum.z expn)
  | _ -> None
...
let sqrt ?(rm=Nearest_even) a =
  ...
  | Fin x ->
    ...
    let s = create desc ~expn:(Bignum.z expn) (Bignum.z frac) in
